  ...
  load: async function(config) {
    ...
    return {
      estimateFaces: async function(input) {
        ...
        // Add specific eye landmarks for better tracking with more realistic movement
        // Left eye
        ...
        // Make iris movements more responsive to mouse position
        const mouseX = (window.mouseX || centerX);
        const mouseY = (window.mouseY || centerY);
        
        // Calculate iris positions based on simulated mouse/cursor position with enhanced sensitivity
        const rightIrisOffsetX = Math.max(-7, Math.min(7, (mouseX - centerX) / 80));  // More range
        const rightIrisOffsetY = Math.max(-5, Math.min(5, (mouseY - centerY) / 80));  // More range
        ...
        // Face contour landmarks with slightly more movement
        mesh[10] = [centerX, centerY - 60 + moveY/1.8, 110]; // Forehead center
        mesh[1] = [centerX, centerY + 25 + moveY/2.5, 150];  // Nose tip
        mesh[454] = [centerX + 70 + moveX/1.8, centerY, 90]; // Right cheek
        mesh[234] = [centerX - 70 + moveX/1.8, centerY, 90]; // Left cheek
        
        // If we have previous positions, blend for smoother motion
        if (lastPositions) {
          const smoothFactor = Math.min(1.0, elapsed / 28); // Smoother transitions
        ...
        // Listen for mousemove to simulate eye tracking more responsively
